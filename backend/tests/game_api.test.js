...
describe('game API', () => {
    ...
    test('POST creates new game (USE_BOCK true)', async () => {
        ...
        expect(response.body.useBock).toBe(true)

        testData.validateResult(response)
    })

    test('POST creates new game (USE_BOCK false)', async () => {
        ...
        expect(response.body.writerId).toMatch(/W[A-Z]{6}/)
        expect(response.body.readerId).toMatch(/R[A-Z]{6}/)
        expect(response.body.useBock).toBe(false)
    ...
    test('POST deal is applied (USE_BOCK true)', async () => {
        process.env.USE_BOCK = true
        const creationResponse = await api.post('/api/game').send(testData.validPlayersSet)
        ...
        expect(bockPreview.single).toBe(7)
        expect(bockPreview.double).toBe(0)
        expect(bockPreview.triple).toBe(0)
    ...
    test('POST deal is applied (USE_BOCK false)', async () => {
        process.env.USE_BOCK = false
        const creationResponse = await api.post('/api/game').send(testData.validPlayersSet)
    ...
    test('POST deal validates deal and fails (USE_BOCK true)', async () => {
        process.env.USE_BOCK = true
        const creationResponse = await api.post('/api/game').send(testData.validPlayersSet)
    ...
    test('POST deal validates deal and fails (USE_BOCK false)', async () => {
        process.env.USE_BOCK = false
        const creationResponse = await api.post('/api/game').send(testData.validPlayersSet)
    ...
    test('POST several deals are applied (USE_BOCK true)', async () => {
        process.env.USE_BOCK = true
        const playersSet = {
            kind: 'playersSet',
            playerNames: [
    ...
    test('Triggering mandatory solo fails (USE_BOCK false)', async () => {
        process.env.USE_BOCK = false
        const playersSet = {
            kind: 'playersSet',
            playerNames: [
        ...
        expect(responseMandatorySoloTrigger.status).toBe(400)
        expect(responseMandatorySoloTrigger.body).toBeDefined()
    })
