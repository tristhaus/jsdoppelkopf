...
describe('game API', () => {
    ...
    test('POST creates new game', async () => {
        ...
        expect(response.body.csvLink).toMatch(new RegExp('http://localhost:3001/api/game/R[A-Z]{6}/csv'))
    ...
    test('Game is accessible to writers by writer ID', async () => {
        ...
        expect(response.body.csvLink).toBeDefined()
    ...
    test('With at least one deal, a CSV can be generated by readerId', async () => {
        const creationResponse = await api.post('/api/game').send(testData.validPlayersSet)
        ...
        const dealResponse = await api.post(writeGamePath).send(deal)

        expect(dealResponse.status).toBe(200)
        expect(dealResponse.body).toBeDefined()
        expect(dealResponse.body.writerId).toBeDefined()
        expect(dealResponse.body.readerId).toBeDefined()

        const writeGameCsvPath = `/api/game/${dealResponse.body.readerId}/csv`

        const response = await api
            .get(writeGameCsvPath)
            .buffer()
            .parse((res, callback) => {
        ...
        expect(csvString).toMatch(/"Name";"Zu zahlen in Euro"/)
        expect(csvString).toMatch(/"A";0,00/)
        expect(csvString).toMatch(/"B";0,00/)
        expect(csvString).toMatch(/"C";0,02/)
        expect(csvString).toMatch(/"D";0,04/)
        expect(csvString).toMatch(/"E";0,02/)
        expect(csvString).toMatch(/"F";0,04/)
        expect(csvString).toMatch(/"G";0,02/)
    })
})
